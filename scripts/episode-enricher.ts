...
async function writeEnrichment(enrichment: Enrichment) {
  const enrichmentsDir = path.join(process.cwd(), 'public', 'data', 'enrichments')
...
async function main() {
  ...
  const newEntries = findNewLongFormEntries(entries, videos)

  if (newEntries.length === 0) {
    console.log(`OK: no new long-form episodes found in YouTube RSS (${entries.length} feed entries).`)
    return
  }

  const newVideos = newEntries.map((entry, index) => videoFromRssEntry(entry, videos, index))
  const dataFilePath = path.join(process.cwd(), 'data', 'videos.ts')
  ...
  if (nextDataFile !== currentDataFile) {
    ...
    console.log(`Updated data/videos.ts with ${newVideos.length} new long-form episode(s).`)
  }

  const pendingSummaries = newVideos.filter(
    (video) =>
      !existsSync(path.join(process.cwd(), 'public', 'data', 'enrichments', `${video.youtubeId}.json`)),
  )

  if (pendingSummaries.length === 0) {
    console.log('OK: all new episodes already have enrichment JSON files.')
    return
  }

  if (!process.env.GROQ_API_KEY) {
    console.error(
      `MISSING: GROQ_API_KEY is not set. Added episode metadata, but skipped ${pendingSummaries.length} new summary file(s).`,
    )
    process.exit(1)
  }

  for (const video of pendingSummaries) {
    process.stdout.write(`Summarizing ${video.youtubeId} ${video.title.slice(0, 60)}... `)
    const enrichment = await summarizeWithGroq(video)
    await writeEnrichment(enrichment)
    console.log('OK')
  }

  console.log(`OK: enriched ${pendingSummaries.length} new episode(s).`)
}
